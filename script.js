...
class Player {
    ...
    update() {
        // Ensure the player character is continuously rendered 
        this.draw();
...
const animate = () => {
    requestAnimationFrame(animate); // The requestAnimationFrame() web API takes in a callback and updates the animation on the screen.
    // As the player moves through the game, you will need to clear the canvas before rendering the next frame of the animation.
    ctx.clearRect(0, 0, canvas.width, canvas.height); // clearRect() Web API takes x, y, width, and height arguments.
    ...
    checkpoints.forEach((checkpoint, index, checkpoints) => {
        const checkpointDetectionRules = [
            ...
            player.position.x - player.width <= checkpoint.position.x - checkpoint.width + player.width * 0.9, // This will ensure that the player is close enough to the checkpoint to claim the checkpoint.
...
// Monitor when the left and right arrow keys are pressed to manage player's movement.
const keys = {
    rightKey: {
        pressed: false
    },
